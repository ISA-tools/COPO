...
def hash_upload(request):
    ...
    file_name = os.path.join(MEDIA_ROOT, file_obj.file.name)
    ...
    study_id = request.GET['study_id']
    ena_collection_id = request.GET['ena_collection_id']
    output_dict['data_file_html'] = htags.generate_study_data_table2(ena_collection_id, study_id)
...
def inspect_file(request):
    ...
    # add file to ENA study
    chunked_upload.type = output_dict['file_type']
    chunked_upload.save()

    study_id = request.GET['study_id']
    ena_collection_id = request.GET['ena_collection_id']
    output_dict['data_file_id'] = EnaCollection().add_file_to_ena_study(study_id, ena_collection_id, file_id)
...
def zip_file(request):
    ...
    study_id = request.GET['study_id']
    ena_collection_id = request.GET['ena_collection_id']
    out['data_file_html'] = htags.generate_study_data_table2(ena_collection_id, study_id)
